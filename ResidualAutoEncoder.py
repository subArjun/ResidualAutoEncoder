...
class ResidualDecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_layers=2, kernel_size=3, stride=2):
        ...
        for i in range(num_layers):
            ...
            layers.append(nn.BatchNorm2d(out_channels))
            layers.append(nn.ReLU(inplace=True))
            layers.append(nn.ConvTranspose2d(out_channels, out_channels, kernel_size, stride=1, padding=1))
            layers.append(nn.BatchNorm2d(out_channels))
        self.block = nn.Sequential(*layers)
...
class ResidualAutoencoder(nn.Module):
    def __init__(self, num_blocks=3, block_depth=2, bottleneck_dim=128, input_size=128):
        ...
        self.ResidualDecoder = ResidualDecoder(num_blocks=num_blocks, block_depth=block_depth, bottleneck_dim=bottleneck_dim, input_size=input_size)
...
def main():
    ...
    model = ResidualAutoencoder(num_blocks=3, block_depth=2, bottleneck_dim=52).to(device)
    criterion = nn.MSELoss()
